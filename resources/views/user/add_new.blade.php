					...
					<form class="form" action="{{ route(currentUser().'.addNewUser') }}" method="POST" enctype="multipart/form-data">
						...
						<div class="card-body">
							...
							<div class="form-group row">
								...
								<div class="col-lg-4">
									<label>User TimeZone: <span class="text-danger sup">*</span></label>
									<select name="timezone" class="form-control select2 tz @if($errors->has('role')) {{ 'is-invalid' }} @endif">
							...
							<div class="form-group row">
                				...
                				<div class="col-lg-4">
                					...
                					<select name="state_id" onchange="get_district(this.value)" class="form-control" required>
                					...
                					<select name="zone_id" class="form-control" required>
                						...
                						@if(count($allZone) > 0)
                							@foreach($allZone as $zone)
                								<option value="{{$zone->id}},{{$zone->code}}" class="dist dist{{$zone->stateId}}">{{ $zone->code.'-'. $zone->name}}</option>
                							@endforeach
                						@endif
                					</select>
