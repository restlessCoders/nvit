...
@section('title', 'Edit Payment Transfer')
@push('styles')
...
<div class="row">
	<div class="col-12">
		<div class="page-title-box">
			...
			<h4 class="page-title">Edit Payment Transfer</h4>
		</div>
	</div>
	<div class="col-12">
		<div class="card-box">
			<form action="{{ route(currentUser().'.payment-transfer.update',$p->id) }}" method="POST" enctype="multipart/form-data">
				@csrf
				<div class="row">
					<div class="col-lg-3">
					...
					<div class="col-lg-3" id="payment-transfer-data">
						...
						<select name="studentId" class="js-example-basic-single form-control select2 @if($errors->has('studentId')) {{ 'is-invalid' }} @endif">
							...
							<option value="{{ $s->id }}" {{ $p->studentId == $s->id ? "selected" : "" }}>{{ $s->id }}-{{ $s->name }}</option>
							@endforeach
							@endif
						</select>
		...
		<script>
			...
			//$('select[name="studentId"]').on('change', function() {
    			var student_id = '{{$p->studentId}}';
				var course_id = '{{$p->course_id}}';
