...
<div class="row">
	...
	<div class="col-12">
		<div class="card-box">
			<form action="{{ route(currentUser().'.batch.update',[encryptor('encrypt', $bdata->id)]) }}" method="POST" enctype="multipart/form-data">
				@csrf
				@method('PUT')
				...
				<div class="form-group row">
					<div class="col-lg-4">
						...
						<select name="courseId" class="js-example-basic-single form-control select2 @if($errors->has('courseId')) {{ 'is-invalid' }} @endif">
							...
							<option value="{{ $course->id }}" {{ old('courseId',$bdata->courseId) == $course->id ? "selected" : "" }}>{{ $course->courseName }}</option>
							@endforeach
							@endif
						</select>
					...
					<div class="col-lg-4">
						...
						<select name="btime" class="js-example-basic-single form-control select2 @if($errors->has('btime')) {{ 'is-invalid' }} @endif">
							...
							<option value="{{ $btime->id }}" {{ old('btime',$bdata->btime) == $btime->id ? "selected" : "" }}>{{ $btime->time }}</option>
							@endforeach
							@endif
						</select>
						@if($errors->has('btime'))
					...
					<div class="col-lg-4">
						...
						<select name="bslot" class="js-example-basic-single form-control select2 @if($errors->has('bslot')) {{ 'is-invalid' }} @endif">
							...
							<option value="{{ $bslot->id }}" {{ old('bslot',$bdata->bslot) == $bslot->id ? "selected" : "" }}>{{ $bslot->slotName }}</option>
							@endforeach
							@endif
						</select>
						@if($errors->has('bslot'))
					...
					<div class="col-lg-4">
						...
						<select name="trainerId" class="js-example-basic-single form-control select2 @if($errors->has('trainerId')) {{ 'is-invalid' }} @endif">
							...
							<option value="{{ $trainer->id }}" {{ old('trainerId',$bdata->trainerId) == $trainer->id ? "selected" : "" }}>{{ $trainer->name }}</option>
							@endforeach
							@endif
						</select>
					...
					<div class="col-lg-4">
						...
						<select name="examRoom" class="js-example-basic-single form-control select2">
							...
							<option value="{{ $c->id }}" {{ old('examRoom',$bdata->examRoom) == $c->id ? "selected" : "" }}>ClassRoom-{{ $c->classroom }}</option>
							@endforeach
							@endif
						</select>
					</div>
					...
					<div class="col-lg-4">
						...
						<div class="input-group">
							<input id="timepicker" type="text" class="form-control" name="examTime" value="{{ old('examTime',$bdata->examTime) }}">
							<div class="input-group-append">
					...
					<div class="col-lg-4">
						...
						<div class="input-group">
							<input id="seat" type="text" class="form-control" name="seat" value="{{ old('seat',$bdata->seat) }}">
							<div class="input-group-append">
								<span class="input-group-text"><i class="mdi mdi-clock-outline"></i></span>
							</div>
						</div><!-- input-group -->
